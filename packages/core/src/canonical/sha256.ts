 ...
 *
 * Canonical hashing has to produce the same digest in a Node API route, a
 * Vitest process, an edge runtime and a browser. `node:crypto` is not available
 * in all of those and `crypto.subtle` is async, which would make every canonical
 * hash in the kernel async. A small synchronous implementation keeps
 * `canonicalHash()` a pure function, which is what the parity tests need.
 */
